...
/// The default in-memory backing store for index and multi-index files.
#[allow(missing_docs)]
...
impl MMap {
    ...
    pub fn map(file: &std::fs::File) -> std::io::Result<Self> {
        ...
        let len = usize::try_from(file.metadata()?.len())
            .map_err(|_| std::io::Error::other("file too large to load into memory"))?;
...
mod mmap {
    ...
    pub fn read_only(path: &Path) -> std::io::Result<super::MMap> {
        Ok(super::MMap(std::fs::read(path)?))
    }
}
