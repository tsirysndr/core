            ...
            Some(directory) => {
                ...
                let outcome = crate::index::write_data_iter_to_stream(
                    index_kind,
                    {
                        ...
                        move || new_pack_file_resolver(data_file, object_hash)
                    },
...
fn resolve_entry(range: data::EntryRange, pack: &crate::data::File, buf: &mut Vec<u8>) -> bool {
    pack.read_into(range, buf)
}
...
fn new_pack_file_resolver(
    ...
    impl Fn(data::EntryRange, &crate::data::File, &mut Vec<u8>) -> bool + Send + Clone,
    crate::data::File,
)> {
    let mut guard = data_file.lock();
    guard.flush()?;
    let path = guard.get_mut().with_mut(|f| f.path().to_owned())?;
    let pack = crate::data::File::at(&path, object_hash)
        .map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err.to_string()))?;
    Ok((resolve_entry, pack))
}
