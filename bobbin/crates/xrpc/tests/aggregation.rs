...
#[tokio::test]
async fn knot_owned_member_is_synthesized_without_slingshot() {
    ...
    assert_eq!(
        items.len(),
        1,
        "synthesized member must hydrate with no slingshot mock mounted"
    );
    ...
    let got = chrono::DateTime::parse_from_rfc3339(
        items[0]["value"]["createdAt"]
            .as_str()
            .expect("createdAt string"),
    )
    .unwrap();
    assert_eq!(got.timestamp_micros(), micros as i64);
}
